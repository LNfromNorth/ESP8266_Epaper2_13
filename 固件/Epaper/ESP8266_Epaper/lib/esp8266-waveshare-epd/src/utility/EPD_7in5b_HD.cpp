...
void EPD_7IN5B_HD_Displaypart(const unsigned char* pbuffer, unsigned long xStart,         unsigned long yStart,\
    ...
    for (unsigned long j = 0; j < height; j++) {
        for (unsigned long i = 0; i < width; i++) {
            if( (j>=yStart) && (j<yStart+Picture_Height) && (i*8>=xStart) && (i*8<xStart+Picture_Width)){
                if(Block == 0){
                    EPD_7IN5B_HD_SendData((pgm_read_byte(&(pbuffer[i-xStart/8 + (Picture_Width)/8*(j-yStart)]))) );
                }else{
                    EPD_7IN5B_HD_SendData(~(pgm_read_byte(&(pbuffer[i-xStart/8 + (Picture_Width)/8*(j-yStart)]))) );
                }
            }else {
